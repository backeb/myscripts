...
year=????
for i in ../AGUa0.10/expt_01.6/data/archm.${year}_???_??.a
do
   ln -s $i AGUarchv.${i:33:11}.a
done

for i in ../AGUa0.10/expt_01.6/data/archm.${year}_???_??.b
do
   ln -s $i AGUarchv.${i:33:11}.b
done
...
do
   ...
   mv $fout.nc AGUdaily_${fout:9:11}.nc
done

rm AGUarchv.*

for i in AGUdaily_*.nc; do tar -zcvf $i.tar.gz $i; done
rm *.nc
